...
export default class Toaxt {
    ...
    new(config) {
        if (!config.duration)
            config.duration = this.defaultDurationMS;
...
function createToast(config) {
    ...
    if (config.icon)
        iconSpan.style.cssText = ToaxtIconCSS;
    ...
    function closeToaxt(ev) {
        wrapperDiv.animate({ transform: ["translateX(0)", "translateX(calc(100% + 15px))"] }, { duration: 250, easing: "ease" }).onfinish = () => {
    ...
    if (config.icon) {
        if (typeof config.icon === "string")
            ...
            iconSpan.append(config.icon);
    }
    if (config.style)
        Object.assign(wrapperDiv.style, config.style);
    if (config.iconStyle)
        Object.assign(iconSpan.style, config.iconStyle);
    if (config.textStyle)
        Object.assign(textSpan.style, config.textStyle);
    if (config.closeStyle)
        Object.assign(closeSpan.style, config.closeStyle);
    wrapperDiv.animate({ transform: ["translateY(100%)", "translateY(0)"] }, { duration: 400, easing: "ease-out" });
    return wrapperDiv;
}
